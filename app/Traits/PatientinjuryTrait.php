...
use App\Models\{Patient_injury,  InjuryDiagnosis, InjuryClaim, AllDocument, WriteOffReason, InjuryContact, InjuryNote, InjuryBill, MasterDataLog, BillPaymentInformation};
...
trait PatientinjuryTrait
{
    ...
    public function saveBillWriteOfReasonInfo($request) {
        ...
        if($reasonInfo){
            ...
            $reasonInfo->for_all_providers    = $request->for_all_providers;
            $reasonInfo->update();
        }
        else{
            $reason = new WriteOffReason();
            $reason->provider_id    = $provider_id;
            $reason->for_all_providers    = $request->for_all_providers;
        ...
        try {
            ...
            if($findInjuryClaim){
                if($request->financial_class == 1){
                    ...
                    $findInjuryClaim->employer_phone_no = $request->employer_phone_number;
                ...
                if($request->financial_class == 1){
                    ...
                    $newInjuryClaim->employer_phone_no = $request->employer_phone_number;
            ...
            if($countArr > 0){
                ...
                if ($checkDiagnos){
                    ...
                    for($i = 0; $i< $countArr; $i++){
                        if($request->work_dg_code_id[$i] != '-Select-' && $request->work_dg_code_id[$i] != ''){
    ...
    public function storeBillPaymentEORDocuments($request, $biilId, $docType) { 
        $checkDoc = AllDocument::where('injury_id', $biilId)->where('doc_type', $docType)->first();
        if($checkDoc){
            $filePathName = public_path('injury_document')."/".$checkDoc->injury_document;
            if (File::exists($filePathName)) {
                File::delete($filePathName);
            }
            $checkDoc->delete();
        ...
        if($request->file('myFile')){
           ...
           $file->move($destinationPath, $filename2);
            ...
            $injuryDocumentId1->is_active	            =  1;
            $injuryDocumentId1->added_by                = Auth::user()->id;
            $injuryDocumentId1->injury_id	            = $biilId;
            $injuryDocumentId1->doc_type	            = $docType;
            $injuryDocumentId1->save();
            $doc_type_msg = 'Updated '. $docType. 'Document';
            $checkBillPaymentInfo = BillPaymentInformation::where('bill_id' , $biilId)->first();
            if($checkBillPaymentInfo){
                $checkBillPaymentInfo->template_document_id = $injuryDocumentId1->id;
                $checkBillPaymentInfo->update();
            }
            $this->addGlobalAllLog('BILL_EOR_DOCUMENT_ADDED','App\AllDocument',$doc_type_msg,$injuryDocumentId1->injury_id);
        } 
    }
}