            ...
            <div class="col-xs-12 col-sm-12 col-md-12">
                <fieldset>
                    <legend><span id="icdId">ICD-{{ ($diagnosesType == 9) ? '9' : '10'}}</span> Diagnosis Codes </legend>
                        @php $i = 1; $leftDignosCnt = 4; $masterCnt =4;  @endphp
                        <div class="form-row cloneDCDiv" id="diagnosesDiv1">
                            @if($pInjuries && $pInjuries->getInjuryClaim && $pInjuries->getInjuryClaim->getInjuryDianoses) 
                                @php $leftDignosCnt = (count($pInjuries->getInjuryClaim->getInjuryDianoses) < 4 ) ? $masterCnt - count($pInjuries->getInjuryClaim->getInjuryDianoses) : count($pInjuries->getInjuryClaim->getInjuryDianoses); 
                                $dC = ( count($pInjuries->getInjuryClaim->getInjuryDianoses) > 0 ) ? count($pInjuries->getInjuryClaim->getInjuryDianoses) +1 : 1;
                                @endphp 
                               @foreach ($pInjuries->getInjuryClaim->getInjuryDianoses as $key =>  $dCode)
                                    <div class="form-group col-md-3 cloneDivDiagnosisCode" id="dio_Code_{{$i}}">
                                        <label for="work_dg_code_id_{{$i}}">   &nbsp;   </label>
                                        ...
                                        @endif
                                        <span id="diagnosError_{{$i}}" class="dc_error"></span>
                                    </div> 
                                    @php $i++; @endphp
                                @endforeach 
                            @endif 
        ...
        <div class="row pt-1 bg-light">
            <div class="col-xs-12 col-sm-12 col-md-12">
                ...
                <fieldset id="cloneDivStr">
                        ...
                        @foreach ($injuryBillInfo->getBillServices as $key =>  $service)
                            <div class="row cloneDiv bg-light p-2 pt-0" id="input_{{$kk}}">
                                ...
                                <div class=" col-md-6">
                                    ...
                                    <table>
                                        <tr>
                                            <td><input onKeypress='checkDiagCodesForServices(event,this.value)' placeholder="Diag Code 1" autocomplete="off" type="text" 
                                            id="bill_diag_codes1{{$kk}}" name="bill_diag_codes1[]" value="{{$service->bill_diag_codes1}}" class="form-control bill_diag_codes1" maxlength="50"></td>
                                            <td><input onKeypress='checkDiagCodesForServices(event,this.value)' placeholder="Diag Code 2" autocomplete="off" type="text" 
                                            id="bill_diag_codes2{{$kk}}" name="bill_diag_codes2[]" value="{{$service->bill_diag_codes2}}" class="form-control" maxlength="50"></td>
                                            <td><input onKeypress='checkDiagCodesForServices(event,this.value)' placeholder="Diag Code 3" autocomplete="off" type="text" 
                                            id="bill_diag_codes3{{$kk}}" name="bill_diag_codes3[]" value="{{$service->bill_diag_codes3}}" class="form-control" maxlength="50"></td>
                                            <td><input onKeypress='checkDiagCodesForServices(event,this.value)'  placeholder="Diag Code 4" autocomplete="off" type="text" 
                                            id="bill_diag_codes4{{$kk}}" name="bill_diag_codes4[]" value="{{$service->bill_diag_code4}}" class="form-control" maxlength="50"></td>
                                        </tr>
                                        <tr>
                                ...
                                <div class="col-md-2 pt-1 pl-0">
                                    <input type="text" placeholder="Unit Amount" class="form-control chargeAmt {{ ($service) ?  ($service->is_master_found == 0 || $service->is_master_found == null ) ? 'd-none' : 'NA' : 'd-none'}}" name="master_charge[]" id="bill_master_charge{{$kk}}" value="{{$service->master_unit_amount}}">
                                </div>
                                <div class="col-md-2 pt-1 pl-0">
                    ...
                    @endif
                        <?php for($i=1; $i <=1; $i++){?>
                            <div class="row cloneDiv bg-light p-2 pt-0" id="input_{{$i}}">
                                <div class=" col-md-2">
                                        <label for="bill_procedure_code">  Procedure Code   </label>
                                        <input onkeyup='searchAutoForProcedureCode(event);' placeholder="Procedure Code" id="bill_procedure_code_{{$i}}" autocomplete="off" type="text" name="bill_procedure_code[]" value=""
                                ...
                                <div class=" col-md-3">
                                    <label for="bill_modifiers">  Modifiers   </label>
                                    <select name="bill_modifiers[]"  multiple onChange='setUnitCodeOnChangeProcedureCode(event,<?php echo $service_lineCnt?>)'   class=" select2-original modefierCode"   id="bill_modifiers{{$i}}" >
                                        <option value="">-Select-</option>
                                        @foreach ($modifiersArray as $modifiyer)
                                ...
                                <div class=" col-md-1">
                                    <label for="bill_units">  Units   </label>
                                    <input placeholder="Units"  autocomplete="off" id="bill_units{{$i}}" type="text" name="bill_units[]" value="" class="form-control bill_unit" maxlength="5">
                                    @if($errors->has('bill_units'))
                                    <span class="invalid-feedback" style="display:block" role="alert">
                                        <strong class="invalid-feedback" >{{ $errors->first('bill_units') }}</strong>
                                    </span>
                                ...
                                <div class=" col-md-6" id="childPt">
                                    ...
                                    <table>
                                        <tr>
                                            ...
                                            <td><input placeholder="Diag Code 2" onKeyup='checkDiagCodesForServices(event,this.value)' autocomplete="off" type="text" id="billDiagCodes2_{{$i}}" name="bill_diag_codes2[]" value="" class="form-control bill_diag_codes2" maxlength="50"></td>
                                            <td><input placeholder="Diag Code 3" onKeyup='checkDiagCodesForServices(event,this.value)' autocomplete="off" type="text" id="billDiagCodes3_{{$i}}" name="bill_diag_codes3[]" value="" class="form-control bill_diag_codes3" maxlength="50"></td>
                                            <td><input placeholder="Diag Code 4" onKeyup='checkDiagCodesForServices(event,this.value)' autocomplete="off" type="text" id="billDiagCodes4_{{$i}}" name="bill_diag_codes4[]" value="" class="form-control bill_diag_codes4" maxlength="50"></td>
                                ...
                                <div class="col-md-2 pt-1 pl-0">
                                    ...
                                    <input placeholder="Unit Amount" type="text" class="form-control chargeAmt d-none" name="master_charge[]" id="bill_master_charge{{$i}}" value="">
                                </div>
                                <div class="col-md-2 pt-1 pl-0">
