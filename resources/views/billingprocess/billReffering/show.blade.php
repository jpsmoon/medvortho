                            ...
                            <ol class="breadcrumb bg-transparent align-self-center m-0 p-0">
                                <li class="breadcrumb-item">
                                    <a class="btn btn-primary"
                                        href="{{ url('billing/referring', $bReferrings->billing_provider_id) }}">
                                        Back</a>
                                </li>
                            </ol>
                        </div>
                    ...
                    <div class="row">
                        <div class="col-md-12">
                            ...
                            <span style="font-size:16px; font-weight:bold">
                               {{ $bReferrings->referring_provider_first_name }}
                                    {{ $bReferrings->referring_provider_last_name ? $bReferrings->referring_provider_last_name : '' }}
                                    {{ $bReferrings->referring_provider_middle_name ? $bReferrings->referring_provider_middle_name : '' }}
                            </span>
                            <br>
                            <span
                                class="provider_heading_type">{{ ($bReferrings->type == 2) ?  'Rendering Provider'  : 'Supervising Provider' }}</span>
                        </div>
                    </div>
                    <hr>
                    <div class="row">
                            <div class="col-md-12">
                                <a href="{{ url('/edit/billing/rendering') }}/{{ $bReferrings->billing_provider_id }}/{{ $bReferrings->id }}"
                                    class="btn btn-primary"><i class="icon-note"></i> Edit</span></a>
                            </div>
                            <div class="col-md-12" style="padding-top:10px;">
                                <label for="rendering_provider_npi"> Name :-</label>
                                <span class="bold">
                                    @if ($bReferrings->provider_type == 1)
                                        {{ $bReferrings->referring_provider_first_name }}
                                        {{ $bReferrings->referring_provider_last_name ? $bReferrings->referring_provider_last_name : '' }}
                                        {{ $bReferrings->referring_provider_middle_name ? $bReferrings->referring_provider_middle_name : '' }}
                                    @else
                                        {{ $bReferrings->entity_name }}
                                    @endif
                                </span>
                            </div>
                        ...
                        <div class="col-md-12">
                            <div class="row" style="padding-left:10px">
                                <div class="col-md-3">
                                    ...
                                    <span class="bold">{{ $bReferrings->referring_provider_npi }}</span>
                                </div>
                                <div class="col-md-3">
                                    <label for="rendering_provider_npi"> Taxonomy Code :-</label>
                                    <span class="bold">{{ $bReferrings->taxonomyCode ? $bReferrings->taxonomyCode->code . ' ' . $bReferrings->taxonomyCode->name : '-' }}</span>
                                </div>
                            </div>
                            ...
                            <div class="row" style="padding-left:10px">
                                    ...
                                    <span class="bold">{{ $bReferrings->provider_type == 1 ? 'Person' : 'Non Person' }}</span>
                                    </div>
                                </div>
                             <hr>   
